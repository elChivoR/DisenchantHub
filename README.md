...
| | |
|---|---|
| **Smart Filtering** | Filter items by rarity (Poor → Legendary) and item level range. Only Armor and Weapons — never touches non-disenchantable items. |
| **Whitelist / Blacklist** | Protect items you want to keep, or mark specific IDs to always disenchant. Right-click any item in the list to manage. |
| **Queue System** | Select a single item or queue everything that passes your filters. One click per disenchant — safe, no taint errors. |
| **Configurable Hotkey** | Default `F5` to toggle the UI. Rebind to any key from the Config tab. |
| **Activity Log** | Timestamped history of every disenchant (success/fail), with stats and a clear button. Persists across sessions. |
...
2. The **Items** tab scans your bags and shows all equipment
3. Green `OK` = passes your filters. Red = blocked (with reason)
4. **Single item**: left-click an item → click the `[DE: ItemName]` button
5. **Bulk**: click `DE Todos` → click `[DE: ...]` once per item in queue
6. Loot is auto-collected. The button updates to the next item automatically.

### Managing Lists

- **Right-click** any item in the Items tab → `Agregar a Whitelist` / `Blacklist`
- Switch to the **Listas** tab to view and remove entries
